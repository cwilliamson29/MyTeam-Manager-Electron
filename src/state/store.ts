...
import { employeeTemplate } from "../helpers/appSettings.tsx";
...
const employeeData = create<EmployeeData>((set) => ({
	...
	removeEmployee: (id: any) => {
		...
		// ters
		const emps = useEmployeeData.getState().employees;
		// remove current employee and temp store
		const empsTemp = emps.filter((emp) => emp.id !== id);
		// set state for new updated employees
		set(() => ({ employees: empsTemp }));
		// remove employee from dexie
		db.employees
			.where("id")
			.equals(id)
			.delete()
